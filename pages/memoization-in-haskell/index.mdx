...
rememeber, Haskell is a pure functional language. There is no mutation, in fact,
...
So using `liftM2`, our code becomes (we also had to add an import):
...
signature. We can also enable the `LambdaCase` extension to clean up its
implementation. With these changes, our code looks like this:
...
both cases - the first one goes nowhere while the second is intant).
