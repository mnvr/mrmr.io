...
The IO monad in Haskell, and declarative style of UI programming popularized by React, have the same essence: instead of writing the program does what we want, write a program that writes the program that does what we want.

In both these contexts, the same essence was arrived at from almost opposite directions - in Haskell from the mathematics of Category theory, in React from the Jungle of JavaScript praxis.

—

Let’s start with React. I feel React is under-appreciated. Every year there is someone who goes out and announces that React is dead. Every year React goes stronger. In fact, its influence is so pervasive that the people who’re often dissing React don’t realize that they themselves are using a framework which is a (usually half-assed) copy of React.

The basic insight of React was - instead of writing the UI directly, we will write a description of the UI. The oldest trick in the programming book - solving a problem by adding another level of indirection.

The unique bit with React is that our description is not meant as a DSL that simplifies writing the UI itself (though React also does that with JSX). What it simplifies is handling state. The most important part of our description is explicitly tell the framework which UI element depends on which “source of truth”.

The framework then takes our description and creates the actual UI. e.g. on the web, it’ll create the actual DOM HTML elements that show things on the web site. So there are two trees

- the view tree (what we wrote)
- the render tree (what the framework constructs from the view tree)

Since we explicitly told React what element in our view tree depends on which state, when the state changes, it knows exactly which part of the render tree it needs to update.

The thing we get out of all this is composability. Every experienced programmer tries to architect their code such that it is composable, because they know that once they have elements that can be composed without thinking of the combinatorial explosion of the interaction between individual parts of the composition, the we can build arbitrarily complex structures (in this case arbitrary complex websites) without things breaking down under the complexity, like what happened to the tower of Babel.

So this is quite elementary. The issue is - composability is really, really, hard to get in practice. That is why over engineering happens - experienced programmers have experienced the power of composition, and want to recreate that in their own architecture	, but they end up designing something that doesn’t _quite_ compose, there is that leaky abstraction somewhere, and the end result is an over engineered mess that nobody apart from them can understand.

What React was able to do was achieve this composability in practice.

The proof of the pudding is in the pie. Not only has React taken over the web world completely (even most React alternatives in the web dev world are at least philosophically inspired by it), it has also taken over Mobile. Flutter and SwiftUI are copies of React. They have branched out yes, gaining their own personalities, but fundamentally they’re just copies.
...
Maybe you can see what I meant by the sadly worse bit above.

To be clear, there are many problems with React, the library. But React, the idea, has absolutely stood the test of time. In fact, a lot of the problems in the JS ecosystem are because of the power of React (the idea) - because of the composability it offers, people are now building bigger and bigger things, and the React the library is often left far behind the state of the art in what people are getting React the idea to do.

—

As an aside, React didn’t click for me until I saw this talk by Dan . My mind was blown when I saw this - Watching it, I felt like I had gained a superpower, and that I could now build any UI that I wanted. While they’re continuation of the same thought, there was something smothering about the class based approach, and I feel it is only with the function components did React (the idea) finally come to its full light.

There are traces of this division in the copies too. Flutter forked React _before_ function components were the vibe, and so it still lives on in the class based (and in my opinion, inferior) approach. SwiftUI forked after function components, and that’s the gene it reflects. At the surface level, SwiftUI might look like it uses the class based approach, but that’s not the case - the SwiftUI structs are actually stateless, and can be thought of as equivalent to the JavaScript functions that form current day React components.

A big eye opener for me was when someone I greatly admire, and who is much more productive than me, said that they still prefer the class based React components. They said they find them simpler to understand.

I still don’t know what to make of it. It goes against the point of this whole article really - that correct abstractions are priceless, and rare. I thought I’ll just mention that what I’m saying is far from a universally held opinion.
