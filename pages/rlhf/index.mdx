...
That said, maybe the analogy is a bit shaky. One good test for the solidity of
an analogy comes from whether the same arrow can be drawn between objects in a
different category (a la category theory). That is, if the analogy, once
obtained, can be extrapolated to a similar but different context, then is a hint
that the analogy is hinting at something fundamental.

But I am unable to do extend this LLM/RLHF vis-a-vis left/right brain analogy to
the other non-human form of intelligence that we know about: corporations.
...
The last one is actually a happy thought, since it means that LLMs with RLHF are
still somewhat analogous to human intelligence, so we can still extrapolate to
comprehend them. Though this would also entail that the day the analogy breaks
is the day we would actually lose track of the children we've birthed.
