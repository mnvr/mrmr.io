...
"overbearing, repressive" RLHF shell that has come to surround the "open-endend,
pure" LLM core. Is RLHF an unneeded historical appendage that is stunting the
...
many creative pursuits <I.Sub>(I'm taking the liberty of bundling together all
the guardrails under the RLHF label)</I.Sub>.
...
independent <I.Sub>(people can live on even after the two hemispheres have been
cut off from each other)</I.Sub>, and control different sides of the
body <I.Sub> (opposite from the side that the hemisphere is in, so the left hemisphere
controls the right side)</I.Sub>. They are posited to give rise to two distinct
...
person going out had to _remember_ what had been said <I.Sub>(This might sound
...
more examples. Here is a relatable example of what it is, or rather, of what it
feels like when it is not there: _flow states_. This mechanism is the part of us
that disappears when we're in a flow state (or the flow state arises when this
mechanism disappears).

A longer term example of too much or too little of this mechanism in play can be
seen in people on the spectrum. This also highlights how in the less extreme
...
tongue. But it is not so mundane - if we were to pull at the tail of this mouse,
we'll find an elephant – our ability to speak stands on this mechanism.
Creativity needs constraints, and this mechanism provides them.

The reason why this mechanism plays a repressive role is because it is easier to
state what not to do rather than what to do. The world is too open ended in
terms of possibilites that it is not feasible to just reinforce all the wanted,
thus we settle in various settings to more and more repress the unwanted: the
rules of sports (not games), the court of law, the commandments of religions.
...
Coming back constraints as creativity thread, this mechanism provides the
necessary context / framework for communication to happen. I presume this is why
adding it to the LLM improved the ability of LLMs to stay on the topic, and come
up with more coherent responses. The language game is not one but two different
games: thought and communication. Both these games use the same primitive of
words, but in one context, thought, we're not constrained and can let our
imagination fly (or rather, imagination is realizing that there are no
constraints), and in the other context, communication, we _need_ to restrict our
imagination and oversee it down to a framework wherein we can meet in the
middle. LLMs are playing the second game, and I presume that is why their
coherence improves by the addition of RLHF.
