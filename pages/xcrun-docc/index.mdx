...
* [Announcement blog post, when DocC was
  open-sourced](https://www.swift.org/blog/swift-docc/)

* [DocC is used to build the Swift book - Announcement blog
  post](https://www.swift.org/blog/tspl-on-docc/)
...
In our Xcode project, use the create new file dialog (`⌘ N`), press tab (`⇥`) to
focus the search field, type "doc", and select the "Documentation Catalog".

It generates a new folder for us, with a single markdown file and an empty
folder. No `Info.plist`! So that's not needed for the minimal setup. We could've
just done `mkdir -p My.docc && touch My.docc/Hello.md` too.

So now that we have the documentation catalog, let us preview it.

```sh
xcrun docc preview Documentation.docc
```

Nice, it runs a preview server for us! We can open it at http://localhost:8080/.
Right now it displays "An unknown error has occurred", but that's fine, it's
better than those "Unexpected Exceptions" at least.

Now let us add some content. Copy paste these ramblings that I've written into
the template `Documentation.docc/Documentation.md` (replacing the existing
content completely).

With a flourish, if we reopen our preview window in the browser, we see, um,,
the same error still. So that's a bit of a let down.
...
Maybe it doesn't like my juvenile, nay kindergarten, sense of humor. Let me give
it a better title. I'll use the name of the project. But still I see the same
error. Worse, docc has even stopped complaining about anything in the Terminal,
it is just silently ignoring us. Maybe we do need that `Info.plist` after all?

Let's copy paste the [Info.plist` from the Swift
Book](https://github.com/apple/swift-book/blob/main/TSPL.docc/Info.plist) into
`Documentation/Info.plist`. Still nothing.
...
.../Documentation.docc/Documentation.md: note: You haven't curated 'doc://org.swift.tspl/documentation/The-Swift-Programming-Language/Documentation'
```
...
From the [Swift-DocC announcement post on Swift
Forums](https://forums.swift.org/t/announcing-swift-docc/52797)
...
StackOverflow. And, indeed, [there we find the
solution](https://stackoverflow.com/questions/76645803/unable-to-build-documentaion-using-docc-preview-file-docc):

> DocC does not provide a "template" on its own. It expects it installed at a
> certain path, that you don't have. To solve that, you need to fetch the
> template and provide a path to it.
...
Yay! It didn't work, but the error changed

> The page you're looking for can't be found.

Sure enough, when I look more closely, the preview URL that docc printed on the
Terminal now has an additional path component. If I open
...
Further, it looks like this problem (of requiring the `@TechnologyRoot`) has
already been fixed on, just [4 days
