 ...
 * any filters down the line to shape. This is subtractive synthesis (start with
 * a waveform that contains a lot of harmonics, and filter them in a musical,
 * time varying, manner).
 */
...
export interface PlayParams {
   ...
   * _actual_ duration of the note is this + attack + decay + release times.
   *
   * In practice, the attack/decay are quite short, and that we think of as the
   * duration is usually the part of the note that sustains. However, to avoid
   * needing recalculations if say a note has a long attack, we use this as the
   * duration of the sustain and add in the attack and decay times.
   *
   * This means though that the default value of this can be potentially
   * misleading. The default value is 50 ms, but it is preceded by a 50 decay
   * (and a 1 ms attack), so what we might expect to be the note duration (the
   * time before it starts to release) is actually more like 100 ms, double the
   * default listed here.
   *
   ...
   *
   * (b) instead of specifying the sustain duration there, we specify it here in
   *     a roundabout way, outside the envelope.
   *
   * However, such specification does map better to how we (musically) want the
   * synth notes to behave.
   *
   * @default 0.05 (50 ms)
   */
  duration?: TSecond;
   ...
   *
   * Note that as a safety measure, there is a hardcoded attenuation (a gain of
   * 0.3) applied to the final signal before sending it to the destination. The
   * level (gain) specified here acts prior to it.
   *
   * In practice, setting this to 1 sounds loud (even after the safety
   * attenuation), so a value of 1 here should be taken as a loud extreme (and a
   * value of 0 as the other extreme of silence).
   *
   * In between is a linear range. The default here is meant as something that
   * is audible even if the user's speakers are set to a low volume, but not too
   * loud if the speakers are turned all the way up (so as to not startle them).
   * But this is more of a prayer than a guarantee, and may not apply based on
   * the user's setup.
   *
   * @default 0.7
   */
   ...
   * Amplitude envelope
   *
   * The sound produced by the synth grows and wanes in amplitude as defined by
   * this amplitude envelope, which is an ADSR {@link Envelope}.
   *
   * The levels in the envelope are scaled by the {@link level} property.
   *
   * - After the attack phase, the envelope reaches 1. The actual sound level at
   *   this point will be `level * 1`.
   *
   * - During the sustain phase, the envelope holds at `sustain`. So the actual
   *   sound level during this time will be `level * sustain`.
   *
 ...
 *   phenomena. For instance, when this envelope is used to control the
 *   amplitude of a note, this sustain duration is the {@link duration} property
 *   of {@link PlayParams}.
 *
...
export interface Envelope {
   ...
   * @default 0.001 (1 ms)
   *
   * For reference, at a sample rate of 44100 Hz, each sample takes 22.67 us. So
   * a ramp of 1 ms would take effect over 44.1 samples.
   */
  attack?: TSecond;
  /**
   ...
   *
   * Note that both this, and the implicit attackLevel (1) is multiplied by the
   * some external (to the envelope) {@link level} parameter when the applying
   * this envelope and figuring out the actual gain values to use.
   *
   * The duration of the sustain phase of the envelope is externally specified.
   * For instance, if an ADSR envelope is used to control the amplitude of a
   * note, then the sustain duration will be equal to the the {@link duration}
   * value in {@link PlayParams}.
   *
   * @default 0.6
   */
  sustain?: Level;
...
export class Synth {
   ...
   * starting audio playback. Trying to create and use an audio context without
   * user interaction results in the browser printing a warning on the console.
   * Worse, it (and this might be undefined behaviour) enqueues any existing
   * events, and then plays them all at once when we do actually trigger an
   * audio context event in response to a user event.
   *
  ...
  async play(params?: PlayParams, onEnded?: () => void) {
    ...
    //
    // Note that we never suspend the audio context. An earlier version of this
    // code did indeed try to be a good citizen and suspend the audio context
    // when the note was done playing. However, doing that caused stuttery
    // audio. So we just let it be. The browser automatically stops showing the
    // "playing"/"speaker" icon next to our tab in the tab bar even if we don't
    // explicitly suspend it, so not suspending doesn't have any functional
    // impact that I can tell of.
    ...
    //
    // But Firefox and Chrome for Android don't support linearRampToValueAtTime.
    amp.gain.setValueCurveAtTime([0, level], t, env.attack);

    // Exponential ramp from `level * 1` to `sustainLevel * level` over `decay`
    // seconds.
    //
    // The third parameter to setTargetAtTime is a `timeConstant`, which is the
    // time it takes to reach 1 - e⁻¹ = 63% of the target (the first parameter).
    //
    // To see where the 63% comes from, notice that 1/e = 0.367. So 1-(e**-1) is
    // 0.632, ~63%. MDN, excellent as always, has a longer
    ...
    //   `target` parameter (the first parameter to setTargetAtTime). The decay
    //   rate as defined by the `timeConstant` parameter (the third parameter)
    //   is exponential; therefore the value will never reach `target`
    //   completely, but after each timestep of length `timeConstant`, the value
    //   will have approach the `target` by another 1 - e⁻¹ ≃ 63%.
    //
    ...
    //     = 1  - e⁻ⁿ
    //
    // So multiplying by 3 gives us 1  - e⁻³ ≃ 0.95, i.e a value that is 95% of
    // the target. Since we already know the time interval we have at our
    ...
    // seconds, after waiting out the ADS phases of the note.
    //
    // This time we scale the timeConstant by 5, to get 99% to the target value.
    // This is needed because we'll remove the node from the audio graph at this
    // point, so having it effectively be silent is necessary so as to not cause
    // clicks.
    amp.gain.setTargetAtTime(
      0,
    ...
    // instanteously applied gain causes clicks in the audio and so in practice
    // we need to use ramps / envelopes when applying them (this is actually
    // true of almost all audio parameters to a certain extent, but is
    // especially true of the output level).
    //
    // So when applying a gain it is better to use a envelope instead. However,
    // since this audio node is meant to serve as a pseudo brick-wall limiter,
    // we start with (and remain at) the constant attenuation, and rely on one
    // of the nodes before us in the chain to apply an envelope to prevent
    // clicks.
    ...
    //
    // WebAudio automatically does "garbage collection" for the entire chain of
    // nodes that were reachable from the source node that we stop. This is
    // described in the (non-normative) "Dynamic Lifetime" section of the
    // WebAudio spec: https://webaudio.github.io/web-audio-api/#DynamicLifetime
    //
    // > The audio system automatically deals with tearing-down the part of the
    //   routing graph for individual "note" events.
    //
    ...
    //   finished playing, the context will automatically release the refernce
    //   to the AudioBufferSourceNode, which in turn will release references to
    //   any nodes it is connected to, and so on. The nodes will automatically
    //   get disconnected from the graph and will be deleted when they have no
    //   more references.
