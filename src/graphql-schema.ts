...
# A tag is specified in the MDX frontmatter as pair of strings - a "tag"
# and a "label".
#
# We have a set of pre-defined tags (defined in 'data/tags.yaml'). To associate
# our post with one of them, we can set the "tag" to the same string as the
# "tag" of one of the entries in 'data/tags.yaml'. Gatsby will foreign-key link
# them together, so in our GraphQL query we can then fetch the data from the
# corresponding TagsYaml entry.
#
# In such a case (when a 'tag' is specified), the label is optional, and if we
# don't provide one, the capitalized TagsYaml's 'tag' will be set as the label
# during parsing (of course, if we do provide it, it'll be used as the label).
#
# Alternatively, we might have tags that are not one of the pre-defined tags.
# In such case, we should not specify a 'tag', and only set the 'label'.
#
# Here are examples of both labelled and unlabelled tags:
#
#     - tags:
#           - tag: programming           # tag + label
#             label: This is a test
#           - tag: programming           # only tag
#           - label: Programming         # only label
#
# See also: Documentation for the FrontmatterTag type in 'parsers/tag.ts'.
#
type MdxFrontmatterTag implements Node @dontInfer {
    tag: TagsYaml @link(by: "tag")
    label: String
}

# The structure of the known tags defined in 'data/tags.yaml'.
#
# For each of these, we create a listing of all the pages that use that
# particular tag under 't/tag' (this value is available as field.slug).
#
# MdxFrontmatterTag entries can optionally have a foreign key @link to these.
type TagsYaml implements Node @dontInfer {
    # This is the "tag" itself. Here it serves as unique id of this tag that
    # we use for @link-ing MdxFrontmatterTag to TagsYaml.
    #
    # This'll be a lowercase, single word, so it can also be used as a short
    # name of the tag.
    #
    # It is not called "id" so as to not conflict with the autogenerated 'id'
    # field added by gatsby-transformer-yaml.
    tag: String!
    # Fields attached by onCreateNode - in particular, the 'slug'.
    fields: TagsYamlFields
    # An optional color associated with this tag.
    color: String
}

type TagsYamlFields {
    # The slug of the page that shows a listing of all the pages with this tag.
    slug: String!
}
`;
